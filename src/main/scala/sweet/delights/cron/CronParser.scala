...
import CronSymbol.{Hash, Wildcard}
...
class CronParser extends RegexParsers:
  ...
  def range: Parser[Range] = "(" ~> ("""\d+""".r ~ ("-" ~> """\d+""".r) <~ ")") ^^ { case start ~ end =>
    start.toInt to end.toInt
  }

  def hashTerm(default: Range): Parser[CronTerm] = "H" ~> range.? ~ step.? ^^ { case range ~ step =>
    val r = range.getOrElse(default)
    val rs = r.start to r.end by step.getOrElse(1)
    CronTerm(Some(Hash), rs)
  }

  def wildcardTerm(default: Range): Parser[CronTerm] = "*" ~> step.? ^^ { case step =>
    CronTerm(Some(Wildcard), step.map(default.start to default.end by _).getOrElse(default))
  }

  def intTerm: Parser[CronTerm] = int ^^ { range => CronTerm(None, range) }
  def rangeTerm: Parser[CronTerm] = range ~ step.? ^^ { case range ~ step =>
  ...
  def spec: Parser[CronExpr] = minute ~ (whiteSpace ~> hour) ~ (whiteSpace ~> day) ~ (whiteSpace ~> month) ~ (whiteSpace ~> dow) ^^ { case m ~ h ~ d ~ mo ~ w =>
    CronExpr.CronSpec(m, h, d, mo, w)
  }
